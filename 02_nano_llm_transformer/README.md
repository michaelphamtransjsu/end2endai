...
```bash
python3 train.py --steps 20 --device cpu
python3 generate.py --checkpoint checkpoints/nano.pt --prompt $'User: hello\nBot:' --tokens 40 --temperature 0.8 --top-k 10
```
...
```bash
python3 train.py --steps 40 --batch-size 4 --seq-len 16 --dim 16 --layers 1 --heads 2 --learning-rate 0.01 --output /tmp/overfit.pt --device cpu
```
